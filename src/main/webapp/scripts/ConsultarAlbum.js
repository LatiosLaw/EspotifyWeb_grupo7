...
async function SuccionarInformacion(tieneSuscripcion) {
    ...
    fetch('ConsultarAlbumServlet?action=devolverInformacionAlbum&albumName=' + encodeURIComponent(primerCampo))
            ...
            .then(data => {
                // Verifica si se recibió un array y tiene al menos un elemento
                if (Array.isArray(data) && data.length > 0) {
                    const album = data[0]; // Obtiene el primer álbum
                    ...
                    // Maneja las imágenes
                    if (!isMobile) {
                        if (album.imagen && (album.imagen.endsWith(".png") || album.imagen.endsWith(".jpg"))) {
                    ...
                    // Limpia la lista de géneros y añade los nuevos
                    ...
                    console.error('No se encontraron datos para el álbum.');
                }
            })
            .catch(error => console.error('Error al cargar los datos del álbum:', error));
